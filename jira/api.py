...
@router.post("/pull", response_model=JiraIssueResponse)
def pull_jira_issue(request: JiraIssueRequest, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        if not upload_success:
            raise HTTPException(
                status_code=500, detail="Failed to upload content to MinIO"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to pull JIRA issue: {str(e)}"
        )
...
@router.post("/project/import", response_model=JiraProjectImportResponse)
def import_jira_project(
    request: JiraProjectImportRequest, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        logger.info(
            f"Processing all issues from project: {request.project_key}"
        )
        ...
        for issue_data in project_data["all_issues"]:
            try:
                ...
                content_data = jira_service.format_issue_for_storage(
                    issue_data, request.include_subtasks
                )
                ...
                if upload_success:
                    ...
                    failed_count += 1
                    logger.error(
                        f"Failed to upload content for issue {content_data['issue_key']}"
                    )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to import JIRA project: {str(e)}"
        )
